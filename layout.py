...
# emblem_model_path = str(current_dir) + "/model/EMBLEM-DocLayout.pt"
# emblem = YOLOv10(emblem_model_path)
...
def get_circular_layout(image_path, device = 'cpu'):
    dets_ptn = get_circular_layout_ptn(image_path, device)
    #dets_emblem = get_circular_layout_emblem(image_path, device)
    #all_dets = dets_ptn + dets_emblem
    dets_ptn.sort(key = lambda x : x[1][1])
    return dets_ptn

def get_circular_layout_ptn(image_path, device = 'cpu'):
    ...
    for entry in det_res:
        ...
        for i in range(len(bboxes)):
            # detect everything except tables
            box = bboxes[i]
            dets.append([classes[i], [int(box[0]), int(box[1]), int(box[2]), int(box[3])]])
    return dets

# def get_circular_layout_emblem(image_path, device = 'cpu'):
#     det_res = emblem.predict(
#     image_path,         # Image to predict
...
#         for i in range(len(bboxes)):
#             box = bboxes[i]
#             dets.append([5, [int(box[0]), int(box[1]), int(box[2]), int(box[3])]])
#     return dets
